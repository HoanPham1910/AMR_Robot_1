...
class EziServoController:
    ...
    def get_actual_position(self) -> int:
        ...
        resp_list = list(resp)
        print("Raw response:", resp_list)  # debug

        # Nếu chưa có _last_position thì khởi tạo
        if not hasattr(self, '_last_position'):
            self._last_position = 0

        # Chỉ xử lý khi đủ 10 byte
        if len(resp_list) == 10:
            # 4 byte vị trí thực tế cuối cùng (little-endian)
            pos_bytes = resp_list[6:10]
            self._last_position = int.from_bytes(pos_bytes, byteorder='little', signed=True)
        else:
            # nếu chuỗi thiếu byte, giữ lại giá trị trước đó
            print(f"⚠️ Frame không đủ 10 byte, giữ lại giá trị cũ: len={len(resp_list)}")
    ...
    def clear_position(self) -> bool:
        ...
        if len(resp) > 0:
            ...
            return False
    #=================================================================
    # CÁC HÀM MỚI CHO THANH GHI CHƯA DÙNG
    #=================================================================

    def get_position_error(self) -> int:
        """
        Đọc sai số giữa command position và actual position (pulse)
        Trả về số xung lệch. Nếu không phản hồi, trả về None.
        """
        resp = self._send_frame(0x54)
        if len(resp) >= 10:
            # sai số nằm ở byte [6:10] (little-endian)
            error_bytes = resp[6:10]
            return int.from_bytes(error_bytes, byteorder='little', signed=True)
        return None

    def set_actual_position(self, position: int) -> bool:
        """
        Đồng bộ hoặc gán vị trí thực tế (pulse) trên driver.
        """
        data = self._int_to_bytes(position)
        resp = self._send_frame(0x52, data)
        return len(resp) > 0

    def move_pause(self, pause_start: bool) -> bool:
        """
        Tạm dừng hoặc tiếp tục động cơ.
        pause_start = True -> bắt đầu pause
        pause_start = False -> giải phóng pause
        """
        data = bytes([1 if pause_start else 0])
        resp = self._send_frame(0x58, data)
        return len(resp) > 0
